...
class AMVE_Item {
	...
	public static function clean_string( $string ) {
		return (string) stripslashes( trim( str_replace( array( '<p>', '</p>' ), '', AMVE_Encoding::UTF8FixWin1252Chars( html_entity_decode( $string, ENT_QUOTES ) ) ) ) );
	}
	...
	public static function is_valid( $item ) {
		return ( $item['code'] || $item['video_url'] )
			&& '' !== $item['id']
			&& '' !== $item['title'];
	}
	...
	public static function get_data_for_json( $item ) {
		...
		if ( 'off' === xbox_get_field_value( 'amve-options', 'import-title' ) ) {
			$item['title'] = '';
		}
		if ( 'off' === xbox_get_field_value( 'amve-options', 'import-description' ) ) {
			$item['desc'] = '';
		}
		if ( 'off' === xbox_get_field_value( 'amve-options', 'import-tags' ) ) {
			$item['tags'] = '';
		}
		if ( 'off' === xbox_get_field_value( 'amve-options', 'import-actors' ) ) {
			$item['actors'] = '';
		}
		...
		return array(
			...
			'thumbs_urls'  => (array) explode( ',', $item['thumbs_urls'] ),
